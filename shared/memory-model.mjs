...
export function buildHistory(e, config = DEFAULT_CONFIG) {
  ...
  for (const rv of (e.reviews || [])) {
    ...
    const peak = Math.min(0.99, r + (1 - r) * g.gain);
    const s = g.stabilityMul === null ? c.S0 : S[L] * g.stabilityMul;
...
export function previewGrade(e, grade, now = 0, config = DEFAULT_CONFIG) {
  ...
  return { peak: m.P[L], stability: m.S[L], days: Math.max(1, Math.round(days)), demoted: grade === 'forget' };
}

/** 留存构成：牢固 ≥ 0.85 / 一般 [DUE_AT, 0.85) / 待复习 < DUE_AT */
export function retentionBreakdown(entries, now = 0, config = DEFAULT_CONFIG) {
  let solid = 0, ok = 0, due = 0;
  for (const e of entries) {
    if (e.muted) continue;
    const r = retentionNow(e, now, config);
    if (r >= 0.85) solid++; else if (r >= config.DUE_AT) ok++; else due++;
  }
  return { solid, ok, due };
}
